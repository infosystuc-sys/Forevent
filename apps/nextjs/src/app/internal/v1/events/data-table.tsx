...
const columns: ColumnDef<ArrayElement<RouterOutputs["web"]["internal"]["pendingEvents"]>>[] = [
    {
        accessorKey: "id",
        header: "",
        cell: () => <></>
    ...
    },
    {
        ...
        cell: ({ row }) => {
            return (
                <p>{row.original.artists.length}</p>
            )
        }
    },
    {
        ...
        cell: ({ row }) => {
            return (
                <p>{row.original.tickets.length}</p>
            )
        }
    },
]

export function DataTable({ data }: { data: RouterOutputs["web"]["internal"]["pendingEvents"] }) {
    const [filtering, setFiltering] = React.useState('')
    const path = usePathname()
    const router = useRouter()

    const getEvents = api.web.internal.pendingEvents.useQuery(undefined, {
        initialData: data,
    })
